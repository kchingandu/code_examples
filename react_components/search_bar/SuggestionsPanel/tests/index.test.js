...
const highlightClass = '.SuggestionsPanel__highlighted';
...
describe('SuggestionPanel', () => {
  ...
  it('should highlight a suggestion item externally through props', () => {
    ...
    assert.equal(suggestionsPanel.find(highlightClass).length, 0);

    suggestionsPanel.setProps({ highlightedItemIndex: 0 });

    assert.equal(suggestionsPanel.find(highlightClass).length, 1);

    assert.isTrue(suggestionsPanel.childAt(0).hasClass('SuggestionsPanel__highlighted'));
  });
  ...
  it('should disable the highlight index on mouse out', () => {
    ...
    const highlightedItemIndex = props.onItemRollover.args[0][0];
