...
import * as resilientURLRequestModuleExports from '@qgo/q-go-services/src/resiliency/httpRequest/resilientHttpRequest';
...
describe('searchSuggestionService', () => {
  ...
  it('should not resolve with an array of suggestions on a failed result', () => {
    const result = { target: { response: '{"suggestion":[{"title":"title 1"}]}' } };

    resilientURLRequestStub.returns({ then: cb => cb(result) });

    searchSuggestionService.search(searchTerm).then((suggestions) => {
      assert.isFalse(suggestions.length);
    });
  });
...
function restoreStubs() {
  URLBuilderModuleExports.default = URLBuilderOriginal;
